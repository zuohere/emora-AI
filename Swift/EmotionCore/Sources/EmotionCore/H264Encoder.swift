...
//  EmotionCore
//
//  H.264 视频编码器（使用VideoToolbox硬件加速）
//
...
public class H264Encoder {
    ...
    public func start() -> Bool {
        ...
        let encodingCallback: VTCompressionOutputCallback = { [weak self] (
            ...
            guard status == noErr, let sampleBuffer = sampleBuffer else {
                os_log("编码错误: %d", log: log, type: .error, status)
                return
            }
            
            if let dataBuffer = CMSampleBufferGetDataBuffer(sampleBuffer),
               let data = CMBlockBufferCopyDataBytes(dataBuffer, atOffset: 0, dataLength: CMBlockBufferGetDataLength(dataBuffer)) {
                ...
                self?.onEncodedData?(data as Data, isKeyframe)
            }
        }
        ...
        guard status == noErr, let session = compressionSession else {
            os_log("创建编码会话失败: %d", log: log, type: .error, status)
            return false
        }
        ...
        VTSessionSetProperty(session, key: kVTCompressionPropertyKey_AverageBitrate, value: NSNumber(value: 500000)) // 500kbps
    ...
    public func encode(pixelBuffer: CVPixelBuffer) {
        ...
        if status != noErr {
            os_log("编码帧失败: %d", log: log, type: .error, status)
        }
    }
